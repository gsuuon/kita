...
type DuckLike< ^a when ^a: (member Walk : unit -> unit) and ^a: (member Quack : unit -> unit) and ^a: (new : unit -> ^a)> =
    ^a
...
type DuckMarker< ^a when ^a: (member Walk : unit -> unit) and ^a: (member Quack : unit -> unit) and ^a: (new :
                                                                                                             unit ->
                                                                                                             ^a)>() =
    class
    end
...
type DuckContainer2<'T> = { foo: string; duck: 'T }
(* static member Empty () = *)
(*   { foo = "" *)
(*     duck = createDuckLike<_, 'T>() } *)

let inline createDuckContainer2<'T, 'a when 'T :> DuckMarker<'a>> () = { foo = ""; duck = new 'a () }

let inline createDuckContainer<'T> () = createDuckContainer2<_, 'T> ()

let doDuckThings (duck: #Duck) = duck.Float()
