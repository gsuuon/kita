type Config(name: string) =
    ...
    member _.Initialize () =
        printfn "Base config initialize"

module Default =
    type Local() =
        inherit Config("Local")
        member _.Initialize () =
            printfn "Local config initialize"
        
    type Az() =
        inherit Config("Azure.Default")
        member _.Initialize () =
            printfn "Az config initialize"

    type Gcp() =
        inherit Config("Gcp.Default")
        member _.Initialize () =
            printfn "Gcp config initialize"

    type Aws() =
        inherit Config("Aws.Default")
        member _.Initialize () =
            printfn "Aws config initialize"
...
module ScratchResource =
    ...
    let inline deployStaticVerify< ^C, ^R when ^R : (member Deploy : ^C -> unit) and ^C :> Config> (resource: ^R, config: ^C)
        =
